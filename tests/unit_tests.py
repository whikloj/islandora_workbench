...
class TestGeneralTests(unittest.TestCase):
    ...
    def test_alt_text_field_config(self):
        """Tests that alt_text_fields config is read properly."""
        config_file_path = os.path.join(
            os.path.dirname(os.path.abspath(__file__)),
            "assets",
            "config_test",
            "test_alt_text_fields.yml",
        )
        args = argparse.Namespace(
            config=config_file_path,
            check=False,
        ...
        expected_config = {
            ...
            "password": "test_password",
            "host": "https://example.com",
            "alt_text_fields": {"field_media_file_alt_text": "field_alternative_text"},
        }
        for key, value in expected_config.items():
            if isinstance(value, dict):
                self.assertDictEqual(value, config[key])
    ...
    def test_image_alt_text_input(self):
        """Tests that image alt text fields are validated properly."""
        config_file_path = os.path.join(
            os.path.dirname(os.path.abspath(__file__)),
            "assets",
            "config_test",
        ...
        config_object = WorkbenchConfig(args)
        config = config_object.get_config()
        config["input_csv"] = "test_alt_text_fields.csv"
        config["input_dir"] = Path(__file__).resolve().parent / "assets/config_test"
        captured_out = io.StringIO()
        captured_err = io.StringIO()
        check_input_exit_code = None
        with contextlib.redirect_stdout(captured_out), contextlib.redirect_stderr(
            captured_err
        ):
            try:
                workbench_utils.check_input(config, args)
            except SystemExit as e:
                check_input_exit_code = e.code
        check_input_output = {
            "stdout": captured_out.getvalue(),
            "stderr": captured_err.getvalue(),
            "exit_code": check_input_exit_code,
        }
        self.assertEqual(check_input_output["exit_code"], 0)

        config["alt_text_fields"]["field_media_file_alt_text"] = "non_existent_column"
        with contextlib.redirect_stdout(captured_out), contextlib.redirect_stderr(
            captured_err
        ):
            try:
        ...
        expected_message = (
            f'Error: For "{config["task"]}" tasks, your CSV file must contain either an "image_alt_text" or have the fields defined'
            f'in your "alt_text_fields" in your CSV file. You are missing the following columns: non_existent_column.'
        )
        self.assertEqual(check_input_output["exit_code"], expected_message)
