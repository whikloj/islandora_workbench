...
from workbench_utils import (
    ...
    WorkbenchCsvReader,
    get_entity_fields,
    check_rollback_file_path_directories,
    ...
    set_media_type,
    ping_media_bundle,
    get_remote_file_extension,
    get_registered_media_extensions,
    get_additional_files_config,
    validate_media_use_tid_in_additional_files_setting,
    check_file_exists,
    paged_content_ignore_file,
    get_sequence_indicator_from_filename,
    validate_weight_value,
    file_is_utf8,
    ping_media,
    ping_term,
    get_file_hash_from_local,
    generate_contact_sheet_from_csv,
)
...
def _simple_config_checks(config: dict):
    """Perform simple checks on the configuration file that don't require any interaction with Drupal or reading the CSV file.
    These checks are performed for all tasks, and if any of them fail, the process exits immediately.
    Parameters
    ...
    if config["task"] not in tasks:
        message = f'"task" in your configuration file must be one of {", ".join(tasks)}, but it is currently set to "{config["task"]}".'
    ...
    ] and workbench_utils.value_is_numeric(
...
def _check_host_values(config: dict, field_names: list):
    """Check to see if there are any "host" column values in the CSV ID to node ID map that aren't empty or the current config["host"] value.
    This is the set of conditions where the map is queried to get parent node IDs. AFAIK it's complete but if others come up, they should be added here.
    Parameters
    :param config: dict - the configuration dictionary
    :param field_names: list - the list of field names in the CSV file, which is used to check for the presence of "parent_id" and whether to query the map for parent IDs
    """
    if (
        len(config["csv_id_to_node_id_map_allowed_hosts"]) > 0
        or (os.environ.get("ISLANDORA_WORKBENCH_SECONDARY_TASKS") is not None)
        or (
            "parent_id" in field_names
        ...
        if check_for_host_column_result[-1][1] == "host":
            ...
            unique_host_values = [
                x[0].strip()
                for x in num_unique_hosts_result
                if x[0] is not None
                and x[0].strip() != ""
                and x[0].strip() != current_host
            ]
...
def _check_get_data_from_view(config: dict, config_filename: str):
    """Perform checks on get_data_from_view tasks. Since this task doesn't use input_dir, input_csv, etc., we exit immediately after doing these checks."""
    # First, ping the View.
    view_parameters = (
...
def _check_media_csv_headers(config: dict, csv_headers: list):
    """Check that the column headers in the CSV file for media-related tasks match Drupal field names for the media type
    being ingested or updated, as well as reserved field names and additional file field names.
    Parameters
    :param config: dict - the configuration dictionary
    :param csv_headers: list - the CSV headers
    """
    field_definitions = workbench_utils.get_field_definitions(
        config, "media", config["media_type"]
    )
    ...
    for csv_column_header in csv_headers:
        if (
            ...
            and csv_column_header not in base_columns
            and csv_column_header not in base_media_fields
            and csv_column_header not in additional
        ):
            logging.error(
...
def task_specific_checks(
    config: dict, reader: workbench_utils.WorkbenchCsvReader, reserved_fields: list
):
    """Perform checks specific to the task defined in the configuration file. These checks are in addition to the more general checks performed in _simple_config_checks() and _check_host_values().
    Parameters
    :param config: dict - the configuration dictionary
    :param reader: WorkbenchCsvReader - the CSV reader object
    :param reserved_fields: list - the list of reserved fields, which is used to check for reserved field names in the CSV column headers
    """
    ...
    if config["task"] == "create":
        ...
        _task_specific_checks_update(
            config, reader, node_base_fields, csv_column_headers
        )
...
def _generate_geolocation_field_list(
    field_definitions: dict, field_names_in_csv: list
) -> list:
    # Generate a list of geolocation fields that are present in the CSV file.
    return [
        key
        for key, val in field_definitions.items()
        if val["field_type"] == "geolocation" and key in field_names_in_csv
    ]


def _generate_link_field_list(
    field_definitions: dict, field_names_in_csv: list
) -> list:
    ...
    return [
        key
        for key, val in field_definitions.items()
        if val["field_type"] == "link" and key in field_names_in_csv
    ]


def _generate_authority_link_field_list(
    field_definitions: dict, field_names_in_csv: list
) -> list:
    ...
    return [
        key
        for key, val in field_definitions.items()
        if val["field_type"] == "authority_link" and key in field_names_in_csv
    ]


def _generate_edtf_field_lists(
    field_definitions: dict, field_names_in_csv: list
) -> list:
    ...
    return [
        key
        for key, val in field_definitions.items()
        if val["field_type"] == "edtf" and key in field_names_in_csv
    ]


def _generate_cardinality_field_list(
    field_definitions: dict, csv_column_headers: list
) -> dict:
    # Generate a list of fields with cardinality >= 1 that are present in the CSV file.
    return {
        key: field_definitions[key]["cardinality"]
        for key in csv_column_headers
        if key in field_definitions
        and field_definitions[key]["cardinality"] > 0
        and key != "title"
    }


def _generate_taxonomy_term_reference_field_list(
    field_definitions: dict, csv_column_headers: list
) -> list:
    ...
    return [
        key
        for key, val in field_definitions.items()
        if val["field_type"] == "entity_reference"
        and val["target_type"] == "taxonomy_term"
        and key in csv_column_headers
    ]


def _generate_typed_relation_field_list(
    field_definitions: dict, csv_column_headers: list
) -> list:
    ...
    return [
        key
        for key, val in field_definitions.items()
        if val["field_type"] == "typed_relation"
        and "typed_relations" in val
        and key in csv_column_headers
    ]


def _task_specific_checks_create(
    config: dict,
    reader: workbench_utils.WorkbenchCsvReader,
    reserved_fields: list,
    node_base_fields: list,
    csv_column_headers: list,
    ...
    if "parent_id" in csv_column_headers:
        ...
        workbench_utils.validate_parent_ids_precede_children(config, reader)
        workbench_utils.prepare_csv_id_to_node_id_map(config)
        if config["query_csv_id_to_node_id_map_for_parents"] is True:
            validate_parent_ids_in_csv_id_to_node_id_map_csv_data = (
                reader.get_csv_data()
            )
        ...
        for csv_column_header in csv_column_headers:
            if (
                ...
                if (
                    ...
                    and csv_column_header not in additional_files_keys
                ):
    ...
    for column_header in csv_column_headers:
        if column_header.startswith("media:"):
            ...
            if (
                media_track_header_parts[1] in config["media_track_file_fields"].keys()
    ...
    if "node_exists_verification_view_endpoint" in config:
        node_exists_config = workbench_utils.get_node_exists_verification_view_endpoint(
            config
        )
    ...
    for csv_column_header in csv_column_headers:
        if (
            ...
            if (
                ...
                entity_reference_view_exists = (
                    workbench_utils.ping_entity_reference_view_endpoint(
                        config,
                        csv_column_header,
        ...
        if len(additional_files) > 0:
            if (
                csv_column_header not in drupal_fieldnames
                and csv_column_header not in node_base_fields
                and csv_column_header not in additional_files
            ):
                if (
                    csv_column_header in config["ignore_csv_columns"]
                    or csv_column_header in additional_files_keys
                ):
                    continue
                logging.error(
    ...
    if (
        config["validate_fixity_during_check"]
    ...
    # Generate a list of taxonomy term reference fields that are present in the CSV file.
    taxonomy_reference_fields_present = _generate_taxonomy_term_reference_field_list(
        field_definitions, reader.get_field_names()
    )
    ...
    # Set this to True if we find any taxonomy term reference fields in the CSV file that are linked to vocabularies that don't exist in Drupal, so we can report at the end of the checks whether there were problems with the taxonomy term reference field configuration.
    has_new_taxonomy_terms: bool = False
    ...
    for taxonomy_reference_field in taxonomy_reference_fields_present:
        try:
            ...
            message = f'Workbench cannot get vocabularies linked to field "{taxonomy_reference_field}". Please confirm that field has at least one vocabulary.'
            taxonomy_reference_errors.append(message)
    if len(taxonomy_reference_errors) > 0:
        error_messages = "\n".join(taxonomy_reference_errors)
        logging.error(
            f"Errors found in taxonomy reference field configuration:\n{error_messages}"
        )
        sys.exit(
    ...
    for row_num, row in enumerate(reader.get_csv_data(), start=1):
        try:
            workbench_utils.validate_required_fields_have_values_action(
                config, required_drupal_fields_node, row
            )
        ...
        if config["nodes_only"] is False and "media_use_tid" in row:
            ...
            for field_value in delimited_field_values:
                if len(field_value.strip()) > 0:
                    try:
                        workbench_utils.validate_media_use_tid(
                            config, field_value, row[config["id_field"]]
                        )
                    except WorkbenchValidationException as e:
                        rows_with_errors.append((row_num, str(e)))
        ...
        if (
            ...
            if "file" in row and "checksum" in row:
                ...
                if not file_name.lower().startswith("http"):
                    ...
                    if (
                        hash_from_local is not False
                        and hash_from_local == row["checksum"].strip()
                    ):
                        logging.info(
        ...
        for geolocation_field in _generate_geolocation_field_list(
            field_definitions, reader.get_field_names()
        ):
            if geolocation_field in row and len(row[geolocation_field]) > 0:
                is_geolocation_fields_present = True
                try:
                    workbench_utils.validate_geolocation_fields_action(
                        config, row, geolocation_field
                    )
                except WorkbenchValidationException as e:
                    rows_with_errors.append((row_num, str(e)))

        for link_field in _generate_link_field_list(
            field_definitions, reader.get_field_names()
        ):
            if link_field in row and len(row[link_field]) > 0:
                link_value = row[link_field].strip()
                is_link_fields_present = True
                delimited_field_values = link_value.split(config["subdelimiter"])
                for field_value in delimited_field_values:
                    if len(
                        field_value.strip()
                    ) > 0 and not workbench_utils.validate_link_value(field_value):
                        message = (
        ...
        for authority_link_field in _generate_authority_link_field_list(
            field_definitions, reader.get_field_names()
        ):
            if authority_link_field in row and len(row[authority_link_field]) > 0:
                ...
                for field_value in delimited_field_values:
                    if len(
                        field_value.strip()
                    ) > 0 and not workbench_utils.validate_authority_link_value(
                        field_value.strip(),
        ...
        for edtf_field in _generate_edtf_field_lists(
            field_definitions, reader.get_field_names()
        ):
            if edtf_field in row and len(row[edtf_field]) > 0:
                is_edtf_fields_present = True
                edtf_value = row[edtf_field].strip()
                delimited_field_values = edtf_value.split(config["subdelimiter"])
                for field_value in delimited_field_values:
                    if len(
                        field_value.strip()
                    ) > 0 and not workbench_utils.validate_edtf_date(
                        field_value.strip()
                    ):
                        message = (
        ...
        for cardinality_field, cardinality_value in _generate_cardinality_field_list(
            field_definitions, reader.get_field_names()
        ).items():
            delimited_field_values = row[cardinality_field].split(
                config["subdelimiter"]
            )
            if len(delimited_field_values) > cardinality_value:
                message = (
                    f'CSV field "{cardinality_field}" in record with ID {row["node_id"]} contains more values '
        ...
        for taxonomy_reference_field in taxonomy_reference_fields_present:
            if len(row[taxonomy_reference_field]) > 0:
                try:
                    if workbench_utils.validate_taxonomy_reference_value_action(
                        config,
                        field_definitions,
                        taxonomy_reference_field,
                        row[taxonomy_reference_field],
                        row[config["id_field"]],
                    ):
                        has_new_taxonomy_terms = True
                except WorkbenchValidationException as e:
        ...
        for typed_relation_reference_field in _generate_typed_relation_field_list(
            field_definitions, reader.get_field_names()
        ):
                    ...
                    if len(field_value.strip()) > 0:
                        try:
                            workbench_utils.validate_typed_relation_field_value_action(
                                config,
                                field_definitions,
                                typed_relation_reference_field,
                                field_value.strip(),
                                row[config["id_field"]],
                            )
                        except WorkbenchValidationException as e:
    ...
    if len(rows_with_errors) > 0:
        error_messages = "\n".join(
            [
                f"Row {row_num}: {error_message}"
                for row_num, error_message in rows_with_errors
            ]
        )
        logging.error(f"Errors found in CSV data:\n{error_messages}")
        sys.exit(
            f"Error: Errors found in CSV data. See log for more detail.\n{error_messages}"
        )
    else:
        if "url_alias" in csv_column_headers:
        ...
        if is_edtf_fields_present:
            ...
            print(message)
            logging.info(message)
        if is_taxonomy_fields_present:
            if has_new_taxonomy_terms and config["allow_adding_terms"]:
            ...
            elif not has_new_taxonomy_terms:
...
def _task_specific_checks_update(
    config: dict,
    reader: workbench_utils.WorkbenchCsvReader,
    node_base_fields: list,
    csv_column_headers: list,
    ...
    for i in special_fields:
        if i in csv_column_headers:
            csv_column_headers.remove(i)
    ...
    # Set this to True if we find any taxonomy term reference fields in the CSV file, so we can report at the end of the checks whether taxonomy term reference field validation was performed.
    is_taxonomy_fields_present = False
    # Set this to True if we find any taxonomy term reference fields in the CSV file that have term IDs/names that do not exist in their respective vocabularies, so we can report at the end of the checks whether there are taxonomy term reference field values that will require new terms to be created.
    has_taxonomy_fields_terms: bool = False
    is_typed_relation_reference_fields_present = False
    ...
    if len(row_errors) > 0:
        ...
        if is_edtf_fields_present:
            ...
            logging.info(message)
        if is_taxonomy_fields_present:
            if has_taxonomy_fields_terms and config["allow_adding_terms"]:
                if config["validate_terms_exist"]:
                    message = "OK, term IDs/names in CSV file exist in their respective taxonomies (new terms will be created"
                    if config["log_term_creation"] is True:
                        message += " as noted in the Workbench log)."
            ...
            elif not has_taxonomy_fields_terms:
                # All term IDs are in their field's vocabularies.
                print(
                    "OK, term IDs/names in CSV file exist in their respective taxonomies."
                )
...
def _task_specific_checks_update_media(
    config: dict, reader: workbench_utils.WorkbenchCsvReader
):
...
def _task_specific_checks_update_media_by_node(
    config: dict, reader: workbench_utils.WorkbenchCsvReader
):
...
def _task_specific_checks_add_media(
    config: dict, reader: workbench_utils.WorkbenchCsvReader
):
...
def _task_specific_checks_create_terms(
    config: dict, reader: workbench_utils.WorkbenchCsvReader, csv_headers: list
):
    ...
    for count, row in enumerate(validate_term_name_csv_data, start=1):
        ...
        for header in csv_headers:
            if (
                "max_length" in field_definitions[header]
                and field_definitions[header]["max_length"] is not None
            ):
                delimited_field_values = row[header].split(config["subdelimiter"])
                for field_value in delimited_field_values:
                    if len(field_value) > int(field_definitions[header]["max_length"]):
                        message = (
                            f'CSV field "{header}" in record with ID {row["node_id"]} contains a value that is '
...
def _task_specific_checks_update_terms(
    config: dict, reader: workbench_utils.WorkbenchCsvReader, csv_column_headers: list
):
...
def _task_specific_checks_add_or_update_alt_text(
    config: dict, reader: workbench_utils.WorkbenchCsvReader
):
    ...
    if len(missing_options) > 0:
        message = (
            ...
            + ", ".join(_alt_text_required_options)
            + "."
        )
        logging.error(message)
        sys.exit("Error: " + message)
    ...
    for row_counter, row in enumerate(reader.get_csv_data(), start=1):
        if len(row["node_id"]) > 0:
            node_id = row["node_id"]
            parent_node_exists = workbench_utils.ping_node(
                config, row["node_id"], warn=False
            )
    ...
    if len(row_warnings) > 0:
        warning_messages = "\n".join(
            [
                f"Row {row_num}: {warning_message}"
                for row_num, warning_message in row_warnings
            ]
        )
        logging.warning(f"Warnings found in CSV data:\n{warning_messages}")
        print(
            f"Warning: Warnings found in CSV data. See log for more detail.\n{warning_messages}"
        )
...
def check_input(config: dict, args: Namespace) -> None:
    ...
    if config["csv_headers"] == "labels" and config["task"] in [
        ...
        mapname_part = (
            f"taxonomy_term-{config['vocab_id']}"
            if config["task"] == "create_terms" or config["task"] == "update_terms"
            else f"node-{config['content_type']}"
        )
        fieldname_map_cache_path = os.path.join(
            config["temp_dir"],
            f"{mapname_part}-labels.fieldname_map",
        )
